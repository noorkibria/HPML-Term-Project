...
[1] Maass, W. (2019). Deep learning in spiking neural networks (Neural Networks).
...
[2] Wu, Y., Deng, L., Li, G., Zhu, J., Xie, Y., & Shi, L. (2018). Spatio‑Temporal Backpropagation for Training High‑Performance Spiking Neural Networks (Frontiers in Neuroscience 12).
...
[3] Zenke, F., & Vogels, T. P. (2019). The remarkable robustness of surrogate gradient learning for instilling complex function in spiking neural networks (Neural Computation 33 (4), 899–925).
...
[4] Wu, Y., Deng, L., Li, G., Zhu, J., Xie, Y., & Shi, L. (2016). Training deep spiking neural networks using backpropagation (Frontiers in Neuroscience 10).
...
[5] Ghosh‑Dastidar, S., & Adeli, H. (2009). Spiking neural networks (International Journal of Neural Systems 19 (4), 295–308).
...
[6] Wu, Y., Deng, L., Li, G., Zhu, J., Xie, Y., & Shi, L. (2019). Direct training for spiking neural networks: faster, larger, better (AAAI Conference on Artificial Intelligence 33 (1), 1311–1318).
...
[7] Zenke, F., & Ganguli, S. (2018). SuperSpike: Supervised learning in multilayer spiking neural networks (Neural Computation 30 (6), 1514–1541).
...
[8] Davies, M., et al. (2024). Exploring neuromorphic computing based on spiking neural networks: algorithms to hardware (ACM Computing Surveys).
...
[9] Zhou, Z., et al. (2025). Semi‑supervised medical image segmentation using spiking neural P‑like convolutional model and pseudo label‑guided cross‑patch contrastive learning (Neurocomputing).
...
[10] Zhang, Y., et al. (2023). Review of medical data analysis based on spiking neural networks (Procedia Computer Science).
...
[11] Khan, A. I., et al. (2025). Spiking neural networks in imaging: a review and case study (Sensors 25 (21)).
...
[12] Wang, H., et al. (2025). Spiking neural network classification of X‑ray chest images (Knowledge‑Based Systems 256).
...
[13] Davies, M., et al. (2024). Neuromorphic computing at scale (Nature).
...
[14] Ahmed, S., et al. (2024). A sustainable neuromorphic framework for disease diagnosis using digital medical imaging (Computer Methods and Programs in Biomedicine Update).
...
[15] Fang, Y., et al. (2025). A fast and accurate ANN–SNN conversion algorithm with negative spikes (International Joint Conference on Artificial Intelligence).
...
[16] Rueckauer, B., Lungu, I.‑A., Hu, Y., Pfeiffer, M., & Liu, S.‑C. (2017). Conversion of continuous‑valued deep networks to efficient event‑driven networks for image classification (Frontiers in Neuroscience 11).
...
[17] Cao, Y., Chen, Y., & Khosla, D. (2015). Spiking deep convolutional neural networks for energy‑efficient object recognition (International Journal of Computer Vision).
...
[18] Kaiser, J., Mostafa, H., & Neftci, E. (2020). Synaptic plasticity dynamics for deep continuous local learning (DECOLLE) (Frontiers in Neuroscience).
...
[19] Patankar, M., Chaurasia, V., & Shandilya, M. (2025). A novel spiking neural network method for classification of tuberculosis using X‑ray images (Computers & Electrical Engineering 122).
...
[20] Kumar, K. A., et al. (2025). A hybrid parallel convolutional spiking neural network for enhanced skin cancer detection (Scientific Reports).
...
[21] Khan, A., et al. (2025). Review of deep learning models with spiking neural networks for modeling and analysis of multimodal neuroimaging data (Frontiers in Neuroscience).
...
[22] Garcia‑Palencia, O., et al. (2025). Spiking neural networks for multimodal neuroimaging: a comprehensive review of current trends and the NeuCube brain‑inspired architecture (Bioengineering).
...
[23] Li, Z., et al. (2025). Brain‑inspired spiking neural networks for energy‑efficient object detection (Conference on Computer Vision and Pattern Recognition).
...
[24] Lee, D., et al. (2025). Spiking transformer with spatial‑temporal attention (Conference on Computer Vision and Pattern Recognition).
...
[25] Shen, H., et al. (2024). Rethinking the membrane dynamics and optimization objectives in spiking neural networks (Neural Information Processing Systems).
...
[26] Lagorce, A., et al. (2017). HOTS: A hierarchy of event‑based time‑surfaces for pattern recognition (IEEE Transactions on Pattern Analysis and Machine Intelligence).
...
[27] Huh, D., & Sejnowski, T. J. (2018). Gradient descent for spiking neural networks (Neural Information Processing Systems).
...
[28] Niu, L.‑Y., Wei, Y., Liu, W.‑B., Long, J.‑Y., & Xue, T.‑H. (2023). Research progress of spiking neural network in image classification: a review (Applied Intelligence 53, 19466–19490).
...
[29] Kheradpisheh, S. R., Ganjtabesh, M., Thorpe, S. J., & Masquelier, T. (2018). STDP‑based spiking deep convolutional neural networks for object recognition (Neural Networks 99, 56–67).
...
[30] Rebecq, H., et al. (2021). High‑speed and high dynamic range video with an event camera (IEEE Transactions on Pattern Analysis and Machine Intelligence).
...
[31] Yu, Q., et al. (2021). Constructing deep spiking neural networks from artificial neural networks with knowledge distillation (AAAI Conference on Artificial Intelligence).
...
[32] Cao, L., et al. (2025). A novel spiking neural network method for classification of tuberculosis using X‑ray images (Computers & Electrical Engineering 122).
URL: https://doi.org/10.1016/j.compeleceng.2024.108044
